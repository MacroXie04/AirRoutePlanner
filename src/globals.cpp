...
void loadData() {
  ...
  for (const auto &r : dbRoutes) {
    if (airportMap.count(r.source_id) && airportMap.count(r.dest_id)) {
      g.addDirectedEdge(airportMap[r.source_id], airportMap[r.dest_id], r.cost,
                        r.duration);
    }
  }
}
...
void refreshDropdowns() {
  if (fromDd && toDd) {
    ...
    if (dbAirports.size() > 0)
      fromDd->value(0);
    if (dbAirports.size() > 1)
      toDd->value(1);
  }
